module.exports = async (client, member) => {
  ...
  jimp.read(url, (err, ava) => {
    ...
    const invite = guildInvites.cache.find(i => gamer.cache.get(i.code).uses < i.uses);
    const inviter = client.users.get(invite.inviter.id);
    const welcome = member.guild.channels.cache.find(channel => channel.name === `${welcome[member.guild.id].channel}`);
